...
#define CYCLE_8_NUM 10000000
#define ANS_BUFFER_LEN 2500000000
...
struct gstruct {           //存儿子和对应权值
    ui son = 0;
    ul val = 0;  // changelog：修改val的类型为ui, 保证结构体gstruct的大小为8字节，64bit，对齐机器字。
...
ui *cycle3[THREAD_COUNT], *cycle4[THREAD_COUNT], *cycle5[THREAD_COUNT], *cycle6[THREAD_COUNT], *cycle7[THREAD_COUNT], *cycle8[THREAD_COUNT];;   //存放对应环
ui *all_cycle[THREAD_COUNT * 6] = {NULL};//存放所有结果
ui cycle_len[THREAD_COUNT * 6] = {0};     //每个结果的环数
ui num_len[THREAD_COUNT * 6] = {0};       //每个结果的数字个数
...
void add_cycle(ui point_stack[], int len, ui id) {
    ...
    switch (len) {
        ...
        case 8: {
            add_id = id + THREAD_COUNT * 5;
            add_len = (cycle_len[add_id]++) * 8;
            for (int j = 0; j < len; ++j) {
                cycle8[id][add_len++] = point_stack[j];
            }
            break;
        }
    }
...
void startUp(ui &start, vector<bool> &set1, vector<bool> &set2, vector<bool> &set3, vector<bool> &set4, vector<ul> &set1_val,
        vector<bool> &visited) {
    visited[start] = true;
    ul val1, val2, val3, val4;
        ...
        for (gstruct &g2 :f[data1]) {
            ...
            for (gstruct &g3 : f[data2]) {
                ...
                if (data3 < start || visited[data3])
                    continue;
                val3 = g3.val;
                if (val2 > 3 * val3 || 5 * val2 < val3)
                    continue;
                set3[data3] = true;
                set4[data3] = true;
                for (gstruct &g4 : f[data3]) {
                    ui data4 = g4.son;
                    if (data4 < start || visited[data4])
                        continue;
                    val4 = g4.val;
                    if (val3 > 3 * val4 || 5 * val3 < val4)
                        continue;
                    set4[data4] = true;
                }
            }
...
void startDown(ui &start, ui id, vector<bool> &set1, vector<bool> &set2, vector<bool> &set3, vector<bool> &set4,
               vector<ul> &set1_val,
               vector<bool> &visited, ui point_stack[]) {
    visited[start] = true;
    point_stack[0] = start;
    // float res = 0;
    ul son1, son2, son3, son4, son5, son6, son7;
    ul val1, val2, val3, val4, val5, val6, val7, addVal;
    for (gstruct &g1 : g[start]) {   //第一层
        ...
        for (gstruct &g2 : g[son1]) { //第二层
            ...
            val2 = g2.val;
            // res = val2 / val1;
            if (val2 > 3 * val1 || 5 * val2 < val1)
                continue;
            point_stack[2] = son2;
            if (set1[son2]) {      //找到长度3的环
                addVal = set1_val[son2];
                if (!((addVal > 3 * val2) || (val1 > 3 * addVal) || (5 * addVal < val2) || (5 * val1 < addVal)))
            ...
            for (gstruct &g3 : g[son2]) {//第三层
                ...
                val3 = g3.val;
                // res = val3 / val2;
                if (val3 > 3 * val2 || 5 * val3 < val2)
                    continue;
                point_stack[3] = son3;
                if (set1[son3]) {
                    //找到4环
                    addVal = set1_val[son3];
                    if (!((addVal > 3 * val3) || (val1 > 3 * addVal) || (5 * addVal < val3) || (5 * val1 < addVal)))
                ...
                for (gstruct &g4 : g[son3]) { //第四层
                    ...
                    if (son4 < start || visited[son4])
                        continue;
                    val4 = g4.val;
                    //                    res = val4 / val3;
                    if (val4 > 3 * val3 || 5 * val4 < val3)
                        continue;
                    point_stack[4] = son4;
                    if (set1[son4]) {
                        //找到5环
                        addVal = set1_val[son4];
                        if (!((addVal > 3 * val4) || (val1 > 3 * addVal) || (5 * addVal < val4) || (5 * val1 < addVal)))
                            add_cycle(point_stack, 5, id);
                    }
                    visited[son4] = true;
                    for (gstruct &g5 : g[son4]) {        //第五层
                        son5 = g5.son;
                        if (!set3[son5]) {     //判断第五层能否继续
                            continue;
                        }
                        if (son5 < start || visited[son5])
                            continue;
                        val5 = g5.val;
                        //                        res = val5 / val4;
                        if (val5 > 3 * val4 || 5 * val5 < val4)
                            continue;
                        point_stack[5] = son5;
                        if (set1[son5]) {
                            //找到6环
                            addVal = set1_val[son5];
                            if (!((val1 > 3 * addVal) || (addVal > 3 * val5) || (5 * addVal < val5) ||
                                  (5 * val1 < addVal)))
                        ...
                        for (gstruct &g6 : g[son5]) {//第六层
                            ...
                            if (son6 < start || visited[son6])
                                continue;
                            val6 = g6.val;
                            if (val6 > 3 * val5 || 5 * val6 < val5)
                                continue;
                            point_stack[6] = son6;
                            addVal = set1_val[son6];
                            if(set1[son6]){
                            	if (!((val1 > 3 * addVal) || (addVal > 3 * val6) || (5 * addVal < val6) ||
                                  (5 * val1 < addVal)))
                                add_cycle(point_stack, 7, id);           //找到长度7的环
                            }                           
                            for (gstruct &g7 : g[son6]) {
                                son7 = g7.son;
                                if (set1[son7]) {
                                    if (visited[son7])
                                        continue;
                                    val7 = g7.val;
                                    if (val7 > 3 * val6 || 5 * val7 < val6)
                                        continue;
                                    point_stack[7] = son7;
                                    addVal = set1_val[son7];
                                    if (!((val1 > 3 * addVal) || (addVal > 3 * val7) || (5 * addVal < val7) ||
                                          (5 * val1 < addVal)))
                                        add_cycle(point_stack, 8, id);           //找到长度7的环
                                }
                            }
                        }
                        visited[son5] = false;
...
void *single_dfs(void *arg) {
    vector<bool> visited(node_cnt, false), set1(node_cnt, false), set2(node_cnt, false), set3(node_cnt, false), set4(
            node_cnt, false);
    vector<ul> set1_val(node_cnt);
    ui point_stack[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    ui id = *(ui *) arg;
    ui start;
    while ((start = atomic_count.fetch_add(JOBS_PER_EPOCH)) < node_cnt) {
        ...
        for (ui i = start; i < end; ++i) {
            ...
            startUp(i, set1, set2, set3, set4, set1_val, visited);
            startDown(i, id, set1, set2, set3, set4, set1_val, visited, point_stack);
        }
    }
    return nullptr;
}
...
void get_sum_len() {
    for (int i = 0; i < THREAD_COUNT * 6; i++) {
        if (i < THREAD_COUNT) {
            sum_len += cycle_len[i] * 3;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 3;
        } else if (i >= THREAD_COUNT && i < 2 * THREAD_COUNT) {
            sum_len += cycle_len[i] * 4;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 4;
        } else if (i >= 2 * THREAD_COUNT && i < 3 * THREAD_COUNT) {
            sum_len += cycle_len[i] * 5;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 5;
        } else if (i >= 3 * THREAD_COUNT && i < 4 * THREAD_COUNT) {
            sum_len += cycle_len[i] * 6;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 6;
        } else if (i >= 4 * THREAD_COUNT && i < 5 * THREAD_COUNT) {
            sum_len += cycle_len[i] * 7;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 7;
        } else {
            sum_len += cycle_len[i] * 8;
            cycle_cnt += cycle_len[i];
            num_len[i] = cycle_len[i] * 8;
        }
    }
}

ui getIndex(ui a, ui b, ui c, ui d, ui e) {
    ui res = 0;
    a < b ?: (a = b, res = 1);
    a < c ?: (a = c, res = 2);
    a < d ?: (a = d, res = 3);
...
void mergeResult() {  //归并排序合并结果,放到一起
    ...
    for (int i = 0; i < 6; i++) {
        int col0 = THREAD_COUNT * i, col1 = THREAD_COUNT * i + 1, col2 = THREAD_COUNT * i + 2, col3 =
                THREAD_COUNT * i + 3, col4 = THREAD_COUNT * i + 4; //列号
        int t[THREAD_COUNT] = {0};//指针指向四个数组
        ui tmp[THREAD_COUNT] = {0};//存放指针所指的值
        ...
        all_cycle[col4][num_len[col4]] = node_cnt + JOBS_PER_EPOCH;
        ...
        while (t[0] < num_len[col0] || t[1] < num_len[col1] || t[2] < num_len[col2] || t[3] < num_len[col3] ||
            ...
            tmp[4] = all_cycle[col4][t[4]];
            minIndex = getIndex(tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);//min(min(tmp0,tmp1),min(tmp2,tmp3));
            node_index = tmp[minIndex];
            switch (i) {
                case 0: {
                    do {
                        for (k = 0; k < 3; k++) {//归并3的环
                            merge_cycle[count++] = all_cycle[THREAD_COUNT * i + minIndex][t[minIndex]];
                            t[minIndex]++;
                        }
...
void writefile(string &writePath) {
    ...
    for (int i = 0; i < THREAD_COUNT; ++i) {
        ...
        lens[5] += cycle_len[THREAD_COUNT * 5 + i];
    }
    ...
    for (int i = 0; i < THREAD_COUNT; ++i) {
        ...
        infos[i].len = 0;
        // 记录当前线程每个长度的环要处理多少个
        infos[i].thread_cycle_cnt = new ui[6];
        // start就是上一个线程的结尾
        infos[i].start = k;
        // 默认分配
        if (i != THREAD_COUNT - 1) {
...
int main() {
    ...
    string filePath = "/data/test_data1887.txt";
    ...
    for (int i = 0; i < THREAD_COUNT; ++i) {                //初始化结果数组
        ...
        all_cycle[i + THREAD_COUNT * 5] = cycle8[i];
    }
#ifdef _TEST
    ...
    cout << "result num " << cycle_cnt << endl;
#endif
    writefile(outPath);
#ifdef _TEST
